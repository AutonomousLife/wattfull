...
import { Search, ChevronDown, ChevronRight, Zap, Sun, Battery, TrendingDown, MapPin, Users, ShieldCheck, ArrowRight, Menu, X, Star, Leaf, DollarSign, Clock, BarChart3, CheckCircle, Award, Globe, ChevronUp } from "lucide-react";

const C = {
  bg: "#FAFAF8", bgAlt: "#F4F3F0", white: "#FFFFFF",
  text: "#2D2D2D", textMid: "#5A5A5A", textLight: "#8A8A8A",
  green: "#5B8C6A", greenLight: "#E8F0EA", greenDark: "#3D6B4A",
  border: "#E8E6E2", borderLight: "#F0EEEA",
  warm: "#F7F6F3", accent: "#C4D4C8",
};

const evSavingsData = [
  { year: "Year 1", ev: 1200, gas: 3800 }, { year: "Year 2", ev: 2400, gas: 7700 },
  { year: "Year 3", ev: 3600, gas: 11800 }, { year: "Year 4", ev: 4800, gas: 16000 },
  { year: "Year 5", ev: 6000, gas: 20400 }, { year: "Year 7", ev: 8500, gas: 29500 },
  { year: "Year 10", ev: 12200, gas: 42000 },
];
const solarROIData = [
  { year: 0, savings: 0, cost: 18500 }, { year: 1, savings: 1850, cost: 18500 },
  { year: 3, savings: 5550, cost: 18500 }, { year: 5, savings: 9250, cost: 18500 },
  { year: 7, savings: 12950, cost: 18500 }, { year: 8, savings: 14800, cost: 18500 },
  { year: 10, savings: 18500, cost: 18500 }, { year: 15, savings: 27750, cost: 18500 },
  { year: 20, savings: 37000, cost: 18500 }, { year: 25, savings: 46250, cost: 18500 },
];
const priceHistoryData = [
  { month: "Jan '24", price: 42500 }, { month: "Mar", price: 41800 },
  { month: "May", price: 40200 }, { month: "Jul", price: 39800 },
  { month: "Sep", price: 38500 }, { month: "Nov", price: 37200 },
  { month: "Jan '25", price: 36800 }, { month: "Mar", price: 35900 },
  { month: "May", price: 35100 }, { month: "Jul", price: 36200 },
  { month: "Sep", price: 34800 }, { month: "Nov", price: 33500 },
  { month: "Jan '26", price: 32800 },
];
const wattScoreData = [
  { subject: "Capacity", A: 92, fullMark: 100 }, { subject: "Value/Watt", A: 85, fullMark: 100 },
  { subject: "Longevity", A: 88, fullMark: 100 }, { subject: "Env. Impact", A: 94, fullMark: 100 },
  { subject: "Efficiency", A: 90, fullMark: 100 }, { subject: "Reliability", A: 87, fullMark: 100 },
];
const stateGrades = [
...
const crowdsourcedData = [
  { month: "Jan", solar: 72, range: 68 }, { month: "Feb", solar: 78, range: 71 },
  { month: "Mar", solar: 85, range: 82 }, { month: "Apr", solar: 91, range: 89 },
  { month: "May", solar: 95, range: 94 }, { month: "Jun", solar: 98, range: 96 },
  { month: "Jul", solar: 100, range: 98 }, { month: "Aug", solar: 97, range: 97 },
  { month: "Sep", solar: 90, range: 92 }, { month: "Oct", solar: 82, range: 85 },
  { month: "Nov", solar: 74, range: 74 }, { month: "Dec", solar: 68, range: 65 },
];

const SectionLabel = ({ children }) => (
  <span style={{ color: C.green, fontSize: 13, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase" }}>{children}</span>
);
const SectionTitle = ({ children }) => (
  <h2 style={{ fontSize: "clamp(28px, 4vw, 42px)", fontWeight: 700, color: C.text, lineHeight: 1.15, margin: "12px 0 0" }}>{children}</h2>
);
const SectionDesc = ({ children }) => (
  <p style={{ fontSize: "clamp(16px, 2vw, 19px)", color: C.textMid, lineHeight: 1.7, margin: "16px 0 0", maxWidth: 620 }}>{children}</p>
);
const Pill = ({ children, active }) => (
  <span style={{
    display: "inline-block", padding: "6px 16px", borderRadius: 100, fontSize: 13, fontWeight: 600,
    background: active ? C.green : C.bgAlt, color: active ? "#fff" : C.textMid,
...
const CustomTooltip = ({ active, payload, label, prefix = "", suffix = "" }) => {
  ...
  return (
    <div style={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
      <p style={{ fontSize: 12, color: C.textLight, margin: 0 }}>{label}</p>
      {payload.map((p, i) => (
        <p key={i} style={{ fontSize: 14, fontWeight: 600, color: p.color || C.text, margin: "4px 0 0" }}>
...
export default function WattfullSite() {
    ...
    const h = () => {
      ...
      for (const id of sections) {
        ...
        if (el) {
          ...
          if (r.top < 300 && r.bottom > 300) { setActiveSection(id); break; }
        }
      }
    };
  ...
  const navItems = [
    { label: "Tools", id: "tools" }, { label: "Reviews", id: "score" },
    { label: "State Reports", id: "states" }, { label: "Research", id: "community" },
  ...
  return (
    <div style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", background: C.bg, color: C.text, minHeight: "100vh", overflowX: "hidden" }}>
      {/* NAV */}
      <nav style={{
        ...
        <div style={{ ...containerStyle, display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
            ...
            <button onClick={() => setMobileMenu(!mobileMenu)} className="nav-mobile-btn" style={{
              background: "none", border: "none", cursor: "pointer", color: C.text, padding: 4, display: "none"
            }}>
            ...
            <div style={containerStyle}>
              {navItems.map(n => (
                <button key={n.id} onClick={() => scrollTo(n.id)} style={{
                  ...
                  padding: "12px 0", fontSize: 16, fontWeight: 500, color: C.text, cursor: "pointer",
      ...
      <section style={{ ...sectionPad, paddingTop: "clamp(80px, 14vw, 160px)", paddingBottom: "clamp(60px, 10vw, 120px)" }}>
        <div style={containerStyle}>
          <div style={{ maxWidth: 780 }}>
            ...
            <h1 style={{ fontSize: "clamp(36px, 6vw, 64px)", fontWeight: 800, lineHeight: 1.08, color: C.text, letterSpacing: "-0.03em", margin: 0 }}>
              Independent Sustainable<br />Energy Intelligence.
            </h1>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 14, marginTop: 40 }}>
              <button onClick={() => scrollTo("ev")} style={{
                background: C.green, color: "#fff", border: "none", borderRadius: 12, padding: "14px 32px",
                fontSize: 15, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 8,
                transition: "transform 0.2s, box-shadow 0.2s", boxShadow: "0 2px 12px rgba(91,140,106,0.25)",
              }}>Run the Numbers <ArrowRight size={16} /></button>
              <button onClick={() => scrollTo("tools")} style={{
                background: "transparent", color: C.text, border: `1.5px solid ${C.border}`, borderRadius: 12,
                padding: "14px 32px", fontSize: 15, fontWeight: 600, cursor: "pointer", transition: "all 0.2s",
              }}>Explore Tools</button>
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 20, marginTop: 72, paddingTop: 48, borderTop: `1px solid ${C.border}` }}>
            {[
        ...
        <div style={containerStyle}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 24, marginTop: 56 }}>
            {[
              ...
              <div key={i} onClick={() => scrollTo(t.link)} style={{
                padding: 32, borderRadius: 16, border: `1px solid ${C.borderLight}`, background: C.bg,
              ...
              >
                <div style={{ width: 48, height: 48, borderRadius: 12, background: C.greenLight, display: "flex", alignItems: "center", justifyContent: "center", color: C.green }}>{t.icon}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 40, marginTop: 48 }}>
            <div>
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                <div>
                  ...
                  <div style={{ position: "relative" }}>
                    <select value={vehicle} onChange={e => setVehicle(e.target.value)}
                      style={{ width: "100%", border: `1.5px solid ${C.border}`, borderRadius: 12, padding: "14px 16px", fontSize: 15, background: C.white, color: C.text, appearance: "none", cursor: "pointer", outline: "none" }}>
                ...
                <button style={{
                  background: C.green, color: "#fff", border: "none", borderRadius: 12, padding: "14px 24px",
            ...
            <div style={{ background: C.bg, borderRadius: 16, border: `1px solid ${C.borderLight}`, padding: "28px 24px" }}>
              ...
              <ResponsiveContainer width="100%" height={320}>
                <AreaChart data={solarROIData}>
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke={C.borderLight} />
                  <XAxis dataKey="year" tick={{ fontSize: 12, fill: C.textLight }} axisLine={false} tickLine={false} label={{ value: "Years", position: "insideBottom", offset: -5, style: { fontSize: 12, fill: C.textLight } }} />
                  <YAxis tick={{ fontSize: 12, fill: C.textLight }} axisLine={false} tickLine={false} tickFormatter={v => `$${(v / 1000).toFixed(0)}k`} />
                  <Tooltip content={<CustomTooltip prefix="$" />} />
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 12, padding: "8px 16px", background: C.greenLight, borderRadius: 8, width: "fit-content", margin: "12px auto 0" }}>
          ...
          <div style={{ marginTop: 48, border: `1px solid ${C.borderLight}`, borderRadius: 16, overflow: "hidden" }}>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14, minWidth: 500 }}>
                ...
                <tbody>
                  {stateGrades.map((s, i) => (
                    <tr key={i} style={{ borderTop: `1px solid ${C.borderLight}`, cursor: "pointer" }}
                      ...
                      <td style={{ padding: "16px", textAlign: "center" }}><GradeBadge grade={s.ev} /></td>
                      <td style={{ padding: "16px", textAlign: "center" }}><GradeBadge grade={s.solar} /></td>
                      <td style={{ padding: "16px", textAlign: "center" }}><GradeBadge grade={s.grid} /></td>
                      <td style={{ padding: "16px", textAlign: "center" }}><GradeBadge grade={s.utility} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <section id="verified" style={{ ...sectionPad, borderTop: `1px solid ${C.border}` }}>
        <div style={containerStyle}>
          <div style={{ maxWidth: 680, margin: "0 auto", textAlign: "center" }}>
            <div style={{
              width: 80, height: 80, borderRadius: 20, background: C.greenLight,
      ...
      <section style={{ background: C.text, padding: "clamp(48px, 8vw, 80px) 0" }}>
        <div style={{ ...containerStyle, textAlign: "center" }}>
          <h2 style={{ fontSize: "clamp(26px, 4vw, 40px)", fontWeight: 800, color: C.white, lineHeight: 1.15, letterSpacing: "-0.02em" }}>Start making smarter energy decisions.</h2>
          ...
          <button onClick={() => scrollTo("ev")} style={{
            background: C.green, color: "#fff", border: "none", borderRadius: 12, padding: "16px 36px",
        ...
        <div style={containerStyle}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 40, marginBottom: 40 }}>
            <div>
              ...
              <p style={{ fontSize: 14, color: C.textMid, lineHeight: 1.6, maxWidth: 280 }}>Independent sustainable energy intelligence for consumers who do their homework.</p>
            </div>
            <div>
              ...
              {["EV Calculator", "Solar ROI", "Wattfull Score", "Price Tracking", "State Reports"].map(l => (
                <div key={l} style={{ fontSize: 14, color: C.textMid, marginBottom: 10, cursor: "pointer" }}>{l}</div>
              ))}
            </div>
            <div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input value={email} onChange={e => setEmail(e.target.value)} placeholder="you@email.com"
                  style={{ flex: 1, border: `1.5px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", fontSize: 14, background: C.white, color: C.text, outline: "none", minWidth: 0 }} />
                <button style={{ background: C.green, color: "#fff", border: "none", borderRadius: 10, padding: "10px 18px", fontSize: 14, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>Subscribe</button>
              </div>
            </div>
          </div>
