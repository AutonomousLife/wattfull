...
async function getGasPrice(zip, state) {
  if (zip) {
    const zipRows = await safeDbCall(() =>
      db.select().from(gasPrices).where(eq(gasPrices.zip, zip)).orderBy(desc(gasPrices.createdAt)).limit(1)
    );
    if (zipRows?.[0]) {
      return {
  ...
  if (state) {
    const stateRows = await safeDbCall(() =>
      db.select().from(gasPrices).where(eq(gasPrices.state, state)).orderBy(desc(gasPrices.createdAt)).limit(1)
    );
    if (stateRows?.[0]) {
      return {
...
function buildOwnershipBreakdown({ years, evMsrp, iceMsrp, evAnnualFuel, iceAnnualFuel, evMaint, iceMaint, incentivesApplied }) {
  const byYear = [];
  ...
  for (let year = 1; year <= years; year += 1) {
    ...
    evCum += evFuelYear + evMaint;
    iceCum += iceFuelYear + iceMaint;
...
export async function calculateComparison(input) {
  ...
  assumptions.push(`Electricity rate: ${round(electricityRateResult.rate, 1)}¢/kWh (${electricityRateResult.source})`);
  ...
  const blendedRatePerKwh =
    chargingMix.homePct * (electricityRateResult.rate / 100) * 1.12 +
    chargingMix.publicPct * (electricityRateResult.rate / 100 + 0.18) * 1.06 +
